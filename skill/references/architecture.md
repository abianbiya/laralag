...
Reads application settings from the `config` database table with caching:

```php
setting('general.app_name')        // returns current_value ?? default_value
setting('display.pagination', 10)  // with fallback
```

- Caches all config rows (keyed by `key`) under `'laralag-config'` for 1 hour
- Cache cleared by `ConfigController::update()` on any save
