...
<div class="row">
    <div class="col-9">
        <form action="{{ route('roleuser.index') }}" method="get">
    ...
    <div class="col-3">
        {{ actionButton('roleuser.create', title: $title) }}
    </div>
</div>
<div class="row mt-2">
    <div class="col-12">
    ...
    <table class="table table-hover mb-0">
        ...
        <tbody>
            ...
            <tr>
				...
				<td>{{ $item->scope->slug }}</td>
				<td>
                    {{ actionButton('roleuser.show', $item->id, $title) }}
                    {{ actionButton('roleuser.edit', $item->id, $title) }}
                    {{ actionButton('roleuser.destroy', $item->id, $title) }}

                    {{-- untuk custom button dapat menggunakan fungsi di bawah atau menggunakan fungsi laravel-html  --}}
                    {{-- {{ customButton('roleuser.blabla', $item->id, $title, 'bi bi-info-square', 'btn-outline-primary') }} --}}
                </td>
            </tr>
            @empty
            <tr>
