...
<div class="row">
    <div class="col-9">
        <form action="{{ route('scope.index') }}" method="get">
    ...
    <div class="col-3">
        {{ actionButton('scope.create', title: $title) }}
    </div>
</div>
<div class="row mt-2">
    <div class="col-12">
    ...
    <table class="table table-hover mb-0">
        ...
        <tbody>
            ...
            <tr>
				...
				<td>{{ $item->kode }}</td>
				<td>
                    {{ actionButton('scope.show', $item->id, $title) }}
                    {{ actionButton('scope.edit', $item->id, $title) }}
                    {{ actionButton('scope.destroy', $item->id, $title) }}

                    {{-- untuk custom button dapat menggunakan fungsi di bawah atau menggunakan fungsi laravel-html  --}}
                    {{-- {{ customButton('scope.blabla', $item->id, $title, 'bi bi-info-square', 'btn-outline-primary') }} --}}
                </td>
            </tr>
            @empty
            <tr>
